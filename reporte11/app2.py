...
# Esto define el título y la configuración de seguridad para el botón "Authorize"
app.config['SWAGGER'] = {
...
def register():
    ...
    Crea una cuenta de usuario con email, username y password, y devuelve los tokens de acceso y refresco.
    ---
    tags:
      - Auth
    parameters:
    ...
    responses:
      ...
      '400':
        description: Datos de entrada faltantes.
      '409':
        description: El email o username ya existen.
    """
...
def login():
    ...
    Autentica a un usuario con su email/username y password. Devuelve nuevos tokens.
    ---
    tags:
      - Auth
    parameters:
...
def refresh():
    ...
    Emite un nuevo access token utilizando un refresh token válido.
    ---
    tags:
      - Auth
    parameters:
...
def logout():
    ...
    Revoca el access token actual y opcionalmente el refresh token proporcionado.
    ---
    tags:
      - Auth
    security:
      ...
      - in: body
        ...
        schema:
          ...
          properties:
            refresh_token:
              type: string
              description: "(Opcional) El refresh token a revocar."
              example: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...'
    responses:
      '200':
...
def introspect():
    ...
    Devuelve el payload decodificado y el estado del token en Redis (allowlist/blacklist).
    ---
    tags:
      - Auth
    parameters:
      - in: body
        ...
        schema:
          ...
          properties:
            token:
              type: string
              description: El access o refresh token a inspeccionar.
    responses:
      '200':
...
def health():
    ...
    Verifica la conexión a la base de datos y Redis.
    ---
    tags:
      - Diagnostics
    responses:
